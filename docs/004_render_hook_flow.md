...
## 4. 각 Hook 함수가 호출될 때 바로 일어나는 일

컴포넌트 함수 안에서 Hook을 호출하면, 각 Hook은 단순히 값을 반환하는 것이 아니라 `hooks[]` 배열의 현재 칸을 읽거나 갱신한다. 이때 기준이 되는 값은 항상 현재의 `hookIndex`다.
...
Hook 호출
  ...
  -> 현재 hookIndex 위치 확인
  -> hookIndex 1 증가
  -> hooks[] 배열의 해당 칸 읽기 또는 갱신
```

### `useState()`가 호출되면

1. 현재 렌더 중인 루트 컴포넌트 인스턴스가 있는지 확인한다.
2. 현재 `hookIndex` 값을 사용해 `hooks[]` 배열의 몇 번째 칸을 사용할지 정한다.
3. 그다음 `hookIndex`를 1 증가시킨다.
4. 그 칸이 비어 있으면 `{ type: 'state', value: initialValue }` 형태로 초기화한다.
5. 그 칸의 `value`를 읽어 현재 state 값으로 반환한다.
6. 함께 반환하는 setter 함수는 나중에 호출되면 같은 칸의 값을 바꾸고 `component.update()`를 실행한다.

즉 `useState()`를 호출하는 순간 하는 일은:

- 현재 사용할 칸 번호를 정하고
- 필요하면 그 칸을 처음 생성하고
- 현재 값을 읽고
- 나중에 같은 칸을 갱신할 setter를 함께 돌려주는 것이다.

### `useMemo()`가 호출되면

1. 현재 렌더 중인 루트 컴포넌트 인스턴스가 있는지 확인한다.
2. 현재 `hookIndex` 값으로 사용할 칸을 정한다.
3. 그다음 `hookIndex`를 1 증가시킨다.
4. 그 칸이 비어 있으면 `fn()`을 즉시 실행해 값을 계산하고, `{ type: 'memo', deps, value }` 형태로 저장한다.
5. 이미 값이 있으면 이전 `deps`와 이번 `deps`를 비교한다.
6. `deps`가 바뀌었으면 `fn()`을 다시 실행해 값을 갱신한다.
7. `deps`가 같으면 저장된 값을 그대로 재사용한다.

즉 `useMemo()`를 호출하는 순간 하는 일은:

- 현재 사용할 칸 번호를 정하고
- 이전 의존성 배열과 이번 의존성 배열을 비교한 뒤
- 필요할 때만 값을 다시 계산하는 것이다.
...
2. 현재 `hookIndex` 값으로 사용할 칸을 정한다.
3. 그다음 `hookIndex`를 1 증가시킨다.
4. 그 칸이 비어 있으면 `{ type: 'effect', deps, pendingFn: fn, needsRun: true }` 형태로 저장한다.
5. 이미 값이 있으면 이전 `deps`와 이번 `deps`를 비교한다.
6. `deps`가 바뀌었으면 `pendingFn = fn`, `needsRun = true`로 바꿔 다시 실행되도록 표시한다.
7. `deps`가 같으면 아무 일도 하지 않고 기존 상태를 유지한다.

중요한 점은 `useEffect()`를 호출하는 순간 effect 본문이 바로 실행되지는 않는다는 것이다.

이 시점에 하는 일은:

- effect를 실행할지 판단하고
- 실행이 필요하면 나중에 실행할 함수(`pendingFn`)를 저장하고
- `needsRun = true`로 표시하는 것까지다.

실제 effect 본문 실행은 렌더와 DOM 반영이 끝난 뒤에 따로 일어난다.

## 5. render 중 자식 함수가 반복 호출되는 방식
...
## 9. `setTodos()` 이후 한 사이클을 실제 앱 기준으로 풀어쓰기
