...
- setState가 호출되면 어떤 순서로 화면이 다시 그려지는가?
...
2. state와 hooks는 루트 컴포넌트인 App에서만 관리합니다.
3. 자식 컴포넌트는 props만 받아서 렌더링하는 Stateless Component로 구성했습니다.
4. 상태 변경 이후에는 전체 DOM을 다시 만드는 대신, 이전 Virtual DOM과 새 Virtual DOM을 비교한 뒤 바뀐 부분만 patch 합니다.
