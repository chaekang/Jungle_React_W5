...
export interface HookDebugSlot {
  index: number;
  type: HookSlot['type'];
  value?: unknown;
  deps?: unknown[];
...
export function getCurrentHookSnapshot(): HookDebugSlot[] {
  const comp = getComponent();

  return comp.hooks.map((slot, index) => {
    if (slot.type === 'state') {
      return {
        index,
        type: slot.type,
    ...
    return {
      ...
      type: slot.type,
      deps: [...slot.deps],
      pending: slot.pendingFn !== null,
      hasCleanup: Boolean(slot.cleanup),
    };
  });
}
