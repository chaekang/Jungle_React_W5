...
describe('patch() — UPDATE_PROPS', () => {
  ...
  it('input value 프로퍼티도 갱신', () => {
    const old = h('input', { value: 'before' });
    const next = h('input', { value: 'after' });
    const dom = createDom(old) as HTMLInputElement;
    dom.value = 'typed';
    patch(dom, diff(old, next));
    expect(dom.value).toBe('after');
  });
});
